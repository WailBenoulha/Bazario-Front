...
const steps = [
  {
    icon: <HiMiniCheckBadge className="h-10 w-10 text-white" />,
    title: "Choose your Niche",
    description: "Pick your market and let Bazario guide you through setting up the perfect store for your audience.",
    step: "01",
    accent: "#E87722",
    perks: ["7 niches available", "AI-powered suggestions", "Trend insights"],
  },
  {
    icon: <LiaStoreSolid className="h-10 w-10 text-white" />,
    title: "Build your Store",
    description: "Customize your layout, branding, and products with our intuitive drag-and-drop builder.",
    step: "02",
    accent: "#2EA7F2",
    perks: ["50+ templates", "Drag & drop builder", "Mobile optimized"],
  },
  {
    icon: <FaMoneyBillTrendUp className="h-10 w-10 text-white" />,
    title: "Start Earning",
    description: "Launch your store and start selling to customers worldwide with built-in payment tools.",
    step: "03",
    accent: "#F4C21F",
    perks: ["Built-in payments", "Real-time analytics", "24/7 support"],
  },
];
    ...
    <div className="py-32 flex flex-col gap-20 items-center px-6 md:px-16 bg-[#fdf8f3] relative overflow-hidden">
      ...
      <section className="flex flex-col items-center gap-6 text-center z-10">
        ...
        <p className="text-gray-500 max-w-md text-base leading-relaxed">
          Get your store up and running in three simple steps — no technical skills required.
        </p>
      </section>
          ...
          {steps.map((step, index) => (
            <div
              ...
              className={`relative flex flex-col gap-6 flex-1 bg-white rounded-3xl p-8 border transition-all duration-500 cursor-default
                ${hovered === step.step
                  ? "shadow-2xl -translate-y-3 border-transparent"
                  : "shadow-md border-gray-100 translate-y-0"
                }`}
              style={{
                boxShadow: hovered === step.step ? `0 20px 60px ${step.accent}25` : undefined,
              }}
            >
              {/* Top glow bar */}
              <div
                className={`absolute top-0 left-8 right-8 h-0.5 rounded-full transition-all duration-500 ${hovered === step.step ? "opacity-100" : "opacity-0"}`}
              ...
              <div className="flex items-center justify-between">
                <span
                  className="text-5xl font-black opacity-10 select-none leading-none"
                ...
                <div
                  className={`hidden md:flex absolute -top-[2.15rem] left-1/2 -translate-x-1/2 h-4 w-4 rounded-full border-2 border-white shadow-md transition-all duration-300 z-10
              ...
              {/* Icon */}
              <div
                className={`self-start p-4 rounded-2xl shadow-lg transition-all duration-300 ${hovered === step.step ? "scale-110" : "scale-100"}`}
              ...
              {index < steps.length - 1 && (
                <div
                  className="hidden md:block absolute -right-5 top-1/2 -translate-y-1/2 z-20 text-lg font-black"
                  style={{ color: step.accent }}
                >
                  →
                </div>
