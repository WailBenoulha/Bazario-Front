...
export default function Home({ onSignUp }: { onSignUp: () => void }) {
  ...
  const stats = [
    { value:"10K+", label: t("hero_stat_1") },
    { value:"98%",  label: t("hero_stat_3") },
    { value:"3min", label: t("hero_stat_2") },
    { value:"50+",  label: t("footer_templates") },
  ];

  return (
    <div dir={isAr ? "rtl" : "ltr"} className="relative min-h-screen bg-[#0a0500] overflow-hidden flex items-center">

      {/* Animated cursor glow */}
      <div className="pointer-events-none fixed inset-0 z-0 transition-opacity duration-300"
        style={{ background:`radial-gradient(600px circle at ${mousePos.x}px ${mousePos.y}px, rgba(232,119,34,0.08), transparent 50%)` }}/>

      {/* Grid pattern */}
      <div className="absolute inset-0 z-0 opacity-10"
        style={{ backgroundImage:`linear-gradient(rgba(244,194,31,0.3) 1px,transparent 1px),linear-gradient(90deg,rgba(244,194,31,0.3) 1px,transparent 1px)`, backgroundSize:"60px 60px" }}/>
      ...
      <div className="relative z-10 w-full flex flex-col lg:flex-row items-center justify-between px-6 md:px-16 py-20 gap-16">
        ...
        <section className={`flex flex-col gap-8 max-w-2xl transition-all duration-1000 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
          ...
          <div className="flex items-center gap-3 self-start">
            ...
            <span className="text-[#E87722] text-xs font-bold tracking-[0.3em] uppercase">{t("hero_badge")}</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <button onClick={onSignUp}
              ...
              <span className="relative z-10">{t("hero_cta_main")}</span>
          ...
          <div className="grid grid-cols-4 gap-4 pt-4 border-t border-white/10">
            {stats.map(stat => (
              <div key={stat.label} className="flex flex-col gap-1">
                <span className="text-2xl font-black bg-gradient-to-r from-[#E87722] to-[#F4C21F] bg-clip-text text-transparent">{stat.value}</span>
        ...
        <div className={`relative flex-shrink-0 transition-all duration-1000 delay-300 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
          ...
          <div className="absolute -top-6 -left-6 z-20 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl px-4 py-3 flex items-center gap-2 shadow-xl">
            ...
            <span className="text-white text-xs font-semibold">{isAr ? "المتجر شغال" : "Store is Live"}</span>
          </div>

          {/* Floating tag 2 */}
          <div className="absolute -bottom-6 -right-6 z-20 bg-gradient-to-r from-[#E87722] to-[#F4C21F] rounded-2xl px-4 py-3 shadow-xl">
            <span className="text-white text-xs font-black">{isAr ? "+124 طلبية اليوم 🚀" : "+124 orders today 🚀"}</span>
          </div>

          {/* Glow behind image */}
          <div className="absolute inset-0 bg-gradient-to-br from-[#E87722]/40 to-[#F4C21F]/20 rounded-3xl blur-3xl scale-110"/>

          <img className="relative z-10 h-64 sm:h-80 md:h-[580px] w-auto rounded-3xl shadow-2xl hover:scale-[1.02] transition-transform duration-500 object-cover"
            src="./src/Pictures/baz-lan.png" alt="Bazario Store Preview"/>
        </div>
      </div>
